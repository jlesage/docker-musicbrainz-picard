...
This project provides a Docker container for [MusicBrainz Picard](https://picard.musicbrainz.org).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of MusicBrainz Picard.
...
```shell
docker stop musicbrainz-picard
```

  2. Remove the container:
```shell
docker rm musicbrainz-picard
```
...
services:
  musicbrainz-picard:
    ...
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/musicbrainz-picard:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-musicbrainz-picard/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/musicbrainz-picard/tags
...
```shell
docker pull jlesage/musicbrainz-picard
```
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/musicbrainz-picard`).
  4.  Select the image, click *Download*, and choose the `latest` tag.
  5.  Wait for the download to complete. A notification will appear once done.
  6.  Click *Container* in the left pane.
  7.  Select your MusicBrainz Picard container.
...
container, it would proxy all HTTP requests for `musicbrainz-picard.domain.tld` to
...
upstream docker-musicbrainz-picard {
...
server {
	...
	server_name musicbrainz-picard.domain.tld;

	location / {
	        proxy_pass http://docker-musicbrainz-picard;
	}
...
server {
	...
	location = /musicbrainz-picard {return 301 $scheme://$http_host/musicbrainz-picard/;}
	location /musicbrainz-picard/ {
		proxy_pass http://docker-musicbrainz-picard/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /musicbrainz-picard/websockify {
			proxy_pass http://docker-musicbrainz-picard/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /musicbrainz-picard/websockify-audio {
			proxy_pass http://docker-musicbrainz-picard/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of MusicBrainz Picard integrated into Docker image. |
...
Any desktop notification generated by MusicBrainz Picard is forwarded to the
